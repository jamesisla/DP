...
class ImpactAssessment(Base, TimestampMixin):
    ...
    base_licitud: Mapped[str] = mapped_column(String(120), default="Obligación Legal (Art. 13)")
    criterios_alto_riesgo_json: Mapped[list] = mapped_column(JSON, default=list)
    ...
    riesgo_residual: Mapped[str] = mapped_column(String(40), default="Aceptable / Bajo")
    opinion_dpo: Mapped[str] = mapped_column(Text, default="")
    estado: Mapped[str] = mapped_column(String(80), default="Aprobado con Mitigaciones")
    dpo_aprobado: Mapped[bool] = mapped_column(Boolean, default=True)
    hash_integridad: Mapped[str] = mapped_column(String(100), default="")
