...
export function CyberAudit({ token }) {
  const [activeTab, setActiveTab] = useState("executive"); // 'executive', 'expediente', 'mock_audit', 'ledger', 'qa'
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex bg-slate-200/70 p-1 rounded-xl border border-slate-300 max-w-3xl overflow-x-auto">
        <button
          ...
          className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${activeTab === "executive" ? "bg-white text-indigo-900 shadow-2xs" : "text-slate-600 hover:text-slate-900"}`}
        >
          Informe Consolidado GRC
        </button>
        <button
          onClick={() => setActiveTab("expediente")}
          className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${activeTab === "expediente" ? "bg-white text-indigo-900 shadow-2xs" : "text-slate-600 hover:text-slate-900"}`}
        >
          Expediente Oficial
        </button>
        <button
          onClick={() => setActiveTab("mock_audit")}
          className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${activeTab === "mock_audit" ? "bg-white text-indigo-900 shadow-2xs" : "text-slate-600 hover:text-slate-900"}`}
        >
          Simulador ANCI
        </button>
        <button
          onClick={() => setActiveTab("ledger")}
          className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${activeTab === "ledger" ? "bg-white text-indigo-900 shadow-2xs" : "text-slate-600 hover:text-slate-900"}`}
        >
          Verificador SHA-256
        </button>
        <button
          onClick={() => setActiveTab("qa")}
          className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${activeTab === "qa" ? "bg-white text-indigo-900 shadow-2xs" : "text-slate-600 hover:text-slate-900"}`}
        >
      ...
      {activeTab === "executive" ? (
        ...
        loadingExecutive ? (
          <div className="rounded-xl border border-line bg-white p-12 text-center text-slate-400">
            <Activity size={32} className="mx-auto mb-2 animate-spin text-indigo-600" />
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-indigo-200 bg-gradient-to-br from-slate-900 via-indigo-950 to-teal-950 text-white p-6 shadow-xl space-y-4">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <a
                    ...
                    download
                    className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-indigo-500 shadow-md transition-all"
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 border-t border-white/10 text-xs">
                <div className="p-3 rounded-xl bg-white/5 border border-white/10 space-y-1">
                  ...
                  <span className="font-bold text-indigo-300">{executiveReport.nivel_cumplimiento}</span>
                </div>
                <div className="p-3 rounded-xl bg-white/5 border border-white/10 space-y-1">
                ...
                <div className="p-3 rounded-xl bg-white/5 border border-white/10 space-y-1">
                  ...
                  <span className="font-bold text-indigo-400 text-sm">{executiveReport.suite_ciberseguridad.score}%</span>
                </div>
                <div className="p-3 rounded-xl bg-white/5 border border-white/10 space-y-1">
                  <span className="text-[10px] text-slate-400 font-bold block uppercase">Madurez Datos (21.719):</span>
                  <span className="font-bold text-teal-300 text-sm">{executiveReport.suite_privacidad.score}%</span>
                </div>
              </div>
            </div>
              ...
              <div className="grid sm:grid-cols-2 gap-3 pt-1">
                {executiveReport.top_prioridades.map((pri, idx) => (
                  <div key={idx} className="p-3 rounded-xl bg-slate-50 border border-slate-200 flex items-start gap-2.5 text-xs text-slate-700">
                    <span className="font-mono font-bold h-5 w-5 rounded-full bg-indigo-100 text-indigo-800 text-[10px] flex items-center justify-center shrink-0 mt-0.5">
                      {idx + 1}
                    </span>
